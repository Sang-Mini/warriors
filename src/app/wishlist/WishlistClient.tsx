  ...
  return (
    <div className="flex items-center gap-2.5 select-none">
      <svg width="30" height="24" viewBox="0 0 38 30" fill="none" aria-hidden>
        <defs>
          <linearGradient id="wGradWishlist" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stopColor={C.primary}   />
            <stop offset="100%" stopColor={C.secondary} />
          </linearGradient>
        </defs>
        <path d="M2 4 L10 26 L19 11 L28 26 L36 4"
          stroke="url(#wGradWishlist)" strokeWidth="4.5"
      ...
      <div style={{ position: "absolute", top: 12, right: 12,
        ...
        <button
          ...
          style={{
            ...
            border: "1px solid #E24B4A", background: "#FFF0F0",
          ...
          <svg width="14" height="14" viewBox="0 0 22 22" fill="none">
            <path d="M11 19C11 19 2 13.5 2 7.5C2 5.01 4.01 3 6.5 3C8.24 3 9.76 3.97 10.65 5.44L11 6L11.35 5.44C12.24 3.97 13.76 3 15.5 3C17.99 3 20 5.01 20 7.5C20 13.5 11 19 11 19Z"
              fill="#E24B4A" stroke="#E24B4A" strokeWidth="1.8" strokeLinejoin="round" />
          </svg>
        </button>
      </div>
...
function EmptyWishlist() {
  ...
  return (
    <div className="flex flex-col items-center justify-center py-28 gap-5 text-center">
      <div style={{ width: 80, height: 80, borderRadius: 28, background: "#FFF0F0",
        display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width="38" height="38" viewBox="0 0 22 22" fill="none" aria-hidden>
          <path d="M11 19C11 19 2 13.5 2 7.5C2 5.01 4.01 3 6.5 3C8.24 3 9.76 3.97 10.65 5.44L11 6L11.35 5.44C12.24 3.97 13.76 3 15.5 3C17.99 3 20 5.01 20 7.5C20 13.5 11 19 11 19Z"
            stroke="#E24B4A" strokeWidth="1.5" strokeLinejoin="round" fill="none" />
        </svg>
      </div>
      <div>
  ...
  return (
    <div className="col-span-full flex flex-col items-center py-28 gap-4 text-center">
      ...
      <div>
        ...
        <p style={{ fontSize: 12, color: C.sub, lineHeight: 1.6, maxWidth: 280, wordBreak: "break-all" }}>
          {message}
        </p>
      </div>
    </div>
...
export default function WishlistClient({
  tournaments: initial, errorMessage,
}: {
  tournaments: Tournament[];
  errorMessage: string | null;
}) {
  const router = useRouter();
  const [items,        setItems]        = useState<Tournament[]>(initial);
  const [toastMsg,     setToastMsg]     = useState("");
  const [toastVisible, setToastVisible] = useState(false);

  const handleRemove = useCallback(async (tournamentId: string) => {
    setItems((prev) => prev.filter((t) => t.id !== tournamentId));
    setToastMsg("찜 목록에서 제거됐어요");
    setToastVisible(true);
    await toggleWishlist(tournamentId, true);
  }, []);
    ...
    <div className="min-h-screen flex flex-col" style={{ background: C.surface }}>
      ...
      <main className="flex-1 pb-16">
        <div className="max-w-[1200px] mx-auto px-6" style={{ boxSizing: "border-box" }}>
          {errorMessage ? (
            <div className="grid"><ErrorState message={errorMessage} /></div>
          ) : items.length === 0 ? (
            <EmptyWishlist />
          ) : (
      ...
      <footer style={{ background: "#f8f7ff",
        borderTop: "0.5px solid rgba(108,60,225,0.12)" }}>
